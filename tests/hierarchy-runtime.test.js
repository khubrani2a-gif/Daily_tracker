...
test("expense hierarchy migrates legacy data and saves predefined and custom subcategories",async()=>{
  ...
  const legacy={version:6,settings:{currency:"SAR",weekStartDay:6,salaryCycleStartDay:27,numberFormat:"ar-EG",updatedAt:now},
    categories:[{id:"food",name:"الأكل",section:"variable",budgets:[{from:"1970-01-01",amountMinor:50000}],isActive:true,sortOrder:0,createdAt:now,updatedAt:now,archivedAt:null}],
    fixedTemplates:[],instances:[],transactions:[{id:"old",amountMinor:1000,transactionDate:today(),categoryId:"food",transactionType:"expense",subcategory:"",createdAt:now,updatedAt:now,deletedAt:null}],trips:[]};
  ...
  try{
    ...
    assert.equal(food.budgets[0].amountMinor,50000);
    ...
    await page.locator("#efAmount").fill("25"); await page.locator("#efCat").selectOption("food");
    ...
    let tx=state.transactions.at(-1); assert.equal(tx.categoryId,"food"); assert.ok(tx.subcategoryId); assert.equal(tx.customSubcategory,null); assert.equal(tx.description,"تموين الأسبوع");

    await page.locator("#expAddBtn").click(); await page.locator("#efAmount").fill("10"); await page.locator("#efCat").selectOption("food");
    ...
    assert.equal(tx.customSubcategory,"منتج موسمي"); assert.equal(state.categories.find(c=>c.id==="food").subcategories.some(s=>s.name==="منتج موسمي"),false);

    await page.locator("#expAddBtn").click(); await page.locator("#efAmount").fill("12"); await page.locator("#efCat").selectOption("food");
    ...
    state=await page.evaluate(()=>JSON.parse(localStorage.getItem("h2do-expenses")));
    assert.equal(state.categories.find(c=>c.id==="food").subcategories.filter(s=>s.name==="قسم المخبز").length,1);
