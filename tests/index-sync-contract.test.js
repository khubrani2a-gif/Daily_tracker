...
test("all nine modules participate in the coordinator",()=>{
  ["pullRemote(currentDate","qPullRemote(","wCustomPullRemote(","witrPullRemote(","hifzPullRemote(","expPullRemote(","wgPullRemote(","rdPullRemote(","learningPullRemote("].forEach(call=>assert.ok(html.includes(call),call));
...
test("optional expense choices retain native select boxes with visual emoji labels",()=>{
  assert.match(html,/const EXP_OPTION_META = \{/);
  ["💳 مدى","🍎 Apple Pay","🏪 من المتجر","🛵 توصيل","🙋 أنا","❤️ الزوج\/ة","👨‍👩‍👧 الأسرة"].forEach(label=>assert.ok(html.includes(label),label));
  assert.match(html,/function expOptions\(arr, sel, meta\)/);
  assert.ok(html.includes("'<option value=\"'+expEsc(v)+'\"'"));
  assert.doesNotMatch(html,/function expChoiceButtons/);
...
test("financial review renders a non-mutating visual risk scan in every period",()=>{
  ...
  assert.match(sw,/const CACHE = "mufakkirati-v130"/);
});
...
test("shared shopping uses the same searchable category pattern as expenses",()=>{
  assert.match(html,/id="familyShopCategorySearch"/);
...
test("weekly goals persist by week, merge by item timestamp, and render on home",()=>{
  ...
  assert.match(html,/SYNC_MODULES = \["daily","quran","customWorship","witr","hifz","expenses","weeklyGoals","reading","learning"\]/);
...
test("expense hierarchy keeps legacy IDs while adding synced subcategories",()=>{
  ...
  assert.match(html,/الدفع والشراء/);
  assert.match(html,/الارتباط والسياق/);
  assert.match(html,/efFxMore/);
